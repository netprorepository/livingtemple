...
require_once('../administrator/setting.php');
 ...
 $curl = curl_init();
         ...
         //  print_r($tranx->data->metadata->fname); exit; 
		   //update transaction and send email	   
			...
			'gateway_response'=>$tranx->data->status,
			);
			...
			//update the application table
                        $application_update_arr=array(								
			'payment_status'=>$tranx->data->status,
			); 
			$whereto= 'where id =' .$tranx->data->metadata->applicant_id;	
			$objComm->db_update_recordm('tbl_application',$application_update_arr,$whereto); 									
