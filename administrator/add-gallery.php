...
<?php include 'include/scriptStyle.php' ?>
 ...
 if(isset($_POST['submit']))
			...
			$date=date("d/m/Y");
			$fields=array('category_id','image','description','status');
			$data=array($category_id,$objComm->uploadfile($_FILES["brand_image"]["name"][$i],$_FILES["brand_image"]["tmp_name"][$i]),$desc,$status);
			$lastId=$objComm->insertWithLastid($fields,$data,'tbl_gallery');				
...
<?php include 'include/leftMenu.php' ?> 
		...
		<form class="form-horizontal row-border" action="" method="post" enctype="multipart/form-data"> 		   
		  ...
		  <?php
		   $protype=$objComm->findAllRecord('tbl_gcategory');
