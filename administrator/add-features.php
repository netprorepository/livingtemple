...
<?php include 'include/scriptStyle.php' ?>
 ...
 if(isset($_POST['submit']))
			...
			extract($_POST);			
			$date=date("d/m/Y");
			$fields=array('title','brand_image','status','date','brand_name','fcategory_id');
			$data=array($title,$objComm->uploadfile($_FILES["brand_image"]["name"],$_FILES["brand_image"]["tmp_name"]),$status,$date,$brand_name,$fcategory_id);
			$lastId=$objComm->insertWithLastid($fields,$data,'tbl_features');				
...
<div id="content"> 
	   ...
	   <li> <a href="javascript:void(0);"><strong>Add Features & Benefits</strong></a> </li> </ul> 
       </div> 
...
<?php include 'include/leftMenu.php' ?> 
		...
		<form class="form-horizontal row-border" action="" method="post" enctype="multipart/form-data"> 		   
		  ...
		  <?php
		  $protype=$objComm->findAllRecord('tbl_fcategory');
