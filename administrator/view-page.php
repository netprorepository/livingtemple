...
<tbody>  	
<?php 
$query=$objComm->findAllRecord('tbl_page');
for($i=0;$i<count($query);$i++)
{
	...
	$aay=array();
			$aay=$objComm->getPostImage($query[$i]['code']);
			$idd=$objComm->getPostImageID($query[$i]['code']);
?>		  
          ...
          <tr id="tr<?=$query[$i][0]?>"> 
			...
			<td><?=$subrow['name']?></td> 
			<td><?=$query[$i]['title']?></td> 
			<td class="back_img"><img src="../products/<?=$aay[0]?>" style="width:100px;" ></td> 
			<td><?=$query[$i]['status']?></td> 
			<td><a href="edit-page.php?id=<?=$query[$i][0]?>" ><i class="icon-pencil"></i></a> &nbsp;&nbsp;<a class="delete" onclick="deletedata('tbl_page','id','<?=$query[$i][0]?>')" ><i class="icon-trash"></i></a></td>
		  </tr> 
